...
def putIntoRedis(urls):
    ...
    # 如果redis有残余url，先清空
    url_number = redis_server.scard('menu_url_1')
    if url_number > 0:
        redis_server.flushall()

    for url in urls:
        redis_server.sadd('menu_url_1', url)
        # 打印redis中url的数目
        url_num = redis_server.scard('menu_url_1')
        # 当前进程PID
        pid = os.getpid()
...
def getAllStep():
    ...
    while(True):
        key_number = redis_server.scard('menu_url_1')
        if key_number > 0:
            url = redis_server.spop('menu_url_1')
            if url != '':
